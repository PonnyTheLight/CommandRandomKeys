...
public class mainCMD implements CommandExecutor {
        ...
        if (sender instanceof Player) {
            ...
            if (args.length == 0) {
                ...
                if (pl.hasPermission("cmdrandomkeys.check")) {
                    if (args.length > 1) {
                        if (keys.contains("Keys." + args[1], true)) {
                ...
                if (pl.hasPermission("cmdrandomkeys.keys")) {
                    ...
                    if (args.length >= 2) {
                        paginate(sender, keyss, Integer.parseInt(args[1]), KeysCount(keyss), 1);
                    } else {
                        paginate(sender, keyss, 1, KeysCount(keyss), 1);
                    }
                } else {
                    pl.sendMessage(ChatColor.translateAlternateColorCodes('&', messages.getString("Messages.PREFIX") + messages.getString("Messages.no-perms")));
                }
                ...
                if (pl.hasPermission("cmdrandomkeys.own.keys")) {
                    ...
                    if (args.length >= 2) {
                        paginate(sender, keyss, Integer.parseInt(args[1]), KeysCount(keyss), 2);
                    } else {
                        paginate(sender, keyss, 1, KeysCount(keyss), 2);
                    }
                } else {
                    pl.sendMessage(ChatColor.translateAlternateColorCodes('&', messages.getString("Messages.PREFIX") + messages.getString("Messages.no-perms")));
                }
    ...
    public void paginate(CommandSender sender, Set<String> list, int page, int countAll, int cmd)
    {
        FileConfiguration messages = plugin.getMessagesConfig();
        FileConfiguration keys = plugin.getKeysConfig();
                        ...
                        if (usesa > useda) {
                            if (diff > 0) {
                                if (cmd == 1) {
                                    ...
                                    keycopy.setClickEvent(new ClickEvent(ClickEvent.Action.COPY_TO_CLIPBOARD, entry));
                                    ...
                                    sender.spigot().sendMessage(keycopy);
